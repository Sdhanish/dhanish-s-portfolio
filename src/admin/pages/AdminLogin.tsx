...
import { ShieldCheck, AlertCircle, ArrowLeft, Loader2, ExternalLink } from 'lucide-react';

export default function AdminLogin() {
  const { currentUser, isAdmin, loginWithGoogle, loginWithGoogleRedirect, authError, clearAuthError } = useAuth();
  const [isLoggingIn, setIsLoggingIn] = useState(false);
  const [authMode, setAuthMode] = useState<'idle' | 'popup' | 'redirect'>('idle');
        ...
        <div className="w-full max-w-md bg-neutral-900/90 backdrop-blur-xl border border-neutral-800 rounded-2xl p-8 shadow-2xl shadow-black/80 space-y-6">
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <span>{isLoggingIn && authMode === 'popup' ? 'Authenticating...' : 'Sign in with Google'}</span>
            </button>

            <button
              onClick={handleGoogleRedirectLogin}
              disabled={isLoggingIn}
              className="w-full py-3 px-4 rounded-xl bg-neutral-800 text-neutral-300 font-medium text-xs hover:bg-neutral-700 hover:text-white transition-all border border-neutral-700 flex items-center justify-center space-x-2 disabled:opacity-50 cursor-pointer"
            >
              {isLoggingIn && authMode === 'redirect' ? (
                <Loader2 className="w-4 h-4 animate-spin text-neutral-400" />
              ) : (
                <ExternalLink className="w-3.5 h-3.5 text-[#BDF869]" />
              )}
              <span>{isLoggingIn && authMode === 'redirect' ? 'Redirecting to Google...' : 'Sign in via Redirect (Desktop/Laptop Backup)'}</span>
            </button>
          </div>
